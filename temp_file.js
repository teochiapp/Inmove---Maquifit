  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.nombre || !formData.telefono || !formData.mail) {
        throw new Error('Por favor, completá todos los campos requeridos.');
      }
      ...
      if (!phoneRegex.test(formData.telefono.replace(/\D/g, ''))) {
        throw new Error('Por favor, ingresá un teléfono válido.');
      }
      ...
      await mercadoPagoService.processPayment(plan, formData);
      
    } catch (error) {
      console.error('Error en el proceso de pago:', error);
      setPaymentError(error.message || 'Error al procesar el pago. Por favor, intentá nuevamente.');
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <ModalHeader>
              <ModalTitle>Completá tus datos</ModalTitle>
              <PlanInfo>
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <ButtonContainer>
                ...
                <PayButton type="submit" disabled={isProcessingPayment}>
                  {isProcessingPayment ? (
                    <>
                      <LoadingSpinner />
                      Procesando...
                    ...
                    `Pagar ${plan?.price?.toLocaleString("es-AR", { 
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0 
                    })}`
                  )}
